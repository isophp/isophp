...
export default class SiderMenu extends PureComponent {
    ...
    getSelectedMenuKeys = (path) => {
        ...
        let paths = [];
        flatMenuKeys.forEach(function (item) {
            paths.push(item.path);
        });
        if (paths.indexOf(path.replace(/^\//, '')) > -1) {
            return [path.replace(/^\//, '')];
        }
        if (paths.indexOf(path.replace(/^\//, '').replace(/\/$/, '')) > -1) {
            return [path.replace(/^\//, '').replace(/\/$/, '')];
        }
        paths = paths.filter((item) => {
            const itemRegExpStr = `^${item.replace(/\//g, '\\\/').replace(/:[\w-]+/g, '([\\w-]+)')}$`;
            const itemRegExp = new RegExp(itemRegExpStr);
            return itemRegExp.test(path.replace(/^\//, ''));
        });
        return paths.map(function (path) {
            let key = '';
            flatMenuKeys.forEach(function (info) {
                if (info.path == path) {
                    key = info.key;
                }
            });
    ...
    getMenuItemPath = (item) => {
        // todo 这里暂时兼容单个参数路由！！
        const itemPath = this.conversionPath(item.key || item.path);
